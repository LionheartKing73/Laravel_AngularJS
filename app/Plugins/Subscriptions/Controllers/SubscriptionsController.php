...
class SubscriptionsController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if($userDetails['is_trial'] == 1 || $userDetails['is_paypal_cancel']){
            $subscriptions = Subscription::where('is_active','=', 1)/*->where('name','!=', 'Free')*/->get();
            return $this->response->item($subscriptions, (new SubscriptionTransformer));
        }else{
            return response()->json(['urlgo'=>'subscribe/list']);
        }
    }
    ...
    public function store(Request $request) {
        $subscription_data = $request->only('user_id', 'education', 'location', 'organization', 'certification_date', 'is_active');
        $user = $this->auth->user();
    ...
    public function update(Request $request, $id) {
        $subscription_data = $request->only('education', 'location', 'organization', 'certification_date');
        $validator = Validator::make($subscription_data, Subscription::GetValidationRule());
        $subscription = false;
        if ($request->has('id')) {
            $subscription = Subscription::find($id);
