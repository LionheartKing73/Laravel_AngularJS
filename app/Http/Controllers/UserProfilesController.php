...
class UserProfilesController extends Controller
{
    ...
    {
        $user_profile_data = $request->only('first_name', 'last_name', 'dr_title', 'practice_name', 'gender_id', 'booking_instructions','about_me', 'board_certifications', 'memberships', 'awards', 'phone', 'address', 'city_id', 'country_id', 'postal_code', 'notification_type_id', 'is_newpatient_accepted', 'dob', 'profileimg');
        $profileImage = $user_profile_data['profileimg'];
        array_pop($user_profile_data);
        ...
        if ($user) {
            if ($validator->passes()) {
                ...
                $city_name= $this->UserProfileService->findGetCityId($user_profile_data['city_id']);
                ...
                try {
                    ...
                    if(!empty($profileImage)){
                        ...
                        if (file_put_contents($path, base64_decode($profileImage))) {
