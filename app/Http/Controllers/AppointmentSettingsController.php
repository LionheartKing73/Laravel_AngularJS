...
class AppointmentSettingsController extends Controller {
    ...
    public function index(Request $request) {
        $user = $this->auth->user();
        $appointmentSettings = AppointmentSettings::where('user_id', '=', $user->id)->first()->toArray();
        $appointmentResponse = ''; 
		$browser = $this->getBrowser();
		$browserTimeZone = $_GET['zone']; 
        $zoneSplit = explode(':', $browserTimeZone);
        foreach ($appointmentSettings as $fieldName => $fieldValue) {
    ...
    public function update(Request $request) {
        ...
        if(empty($errorMessage)){
            $user = $this->auth->user();
            $updateValue = AppointmentSettings::where('user_id', '=', $user->id)->update($storedValue);
            if ($updateValue) {
